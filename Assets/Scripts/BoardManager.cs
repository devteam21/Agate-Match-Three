...
public class BoardManager : MonoBehaviour
{
    ...
    private Dictionary<TileController, int> GetAllDrop()
    {
        ...
        for (int x = 0; x < size.x; x++)
        {
            for (int y = 0; y < size.y; y++)
            {
                ...
                {
                    //process for all tile on top of destroy tile
    ...
    public IEnumerator SwapTilePosition(TileController a, TileController b, System.Action onCompleted)
    {
        ...
        bool isRountineACompleted = false;
        bool isRountineBCompleted = false;

        StartCoroutine(a.MoveTilePosition(GetIndexPosition(indexB), () => { isRountineACompleted = true; }));
        StartCoroutine(b.MoveTilePosition(GetIndexPosition(indexA), () => { isRountineBCompleted = true; }));

        yield return new WaitUntil(() => { return isRountineACompleted && isRountineBCompleted; });
